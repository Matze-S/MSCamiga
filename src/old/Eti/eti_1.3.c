...
/* Etiketten-Druck-Programm ETI 1.3 für EPUCRET BAUCHEMIE */
/* Geschrieben von Matthias Schmidt - Eislingen, November 1989 */
...
/*#ENDFD*/
      ...
      case 2:
         ...
         as("",25);
         inh(7);
         as("\n\n          Mischungsverhältnis: ",0);
         as(mischung,20);
         as("\n\n\n          Produkt enthält: \x0f",0);
         as(enthaelt[0],50);
         for (i = 1; i < 4; ++i) {
            as("\n\x12",29);
            *pbp++ = 15;
...
struct IntuiText groesse_text[] = { {
      ...
      1,0,JAM2,14,1,&Topaz_80,(STRPTR)"Mittelgroß",0
   },{
      1,0,JAM2,14,1,&Topaz_80,(STRPTR)"   Groß   ",0
   }
};
...
struct Gadget inhalt_gg = {
   &enthaelt_gg,-67,GG_Y + 32,48,8,GADGHCOMP|GRELRIGHT,RELVERIFY,
...
struct NewWindow new_wnd = {
   28,13,584,243,-1,-1,GADGETUP|CLOSEWINDOW,WINDOWDEPTH|
   WINDOWCLOSE|SMART_REFRESH|ACTIVATE|NOCAREREFRESH|RMBTRAP,&titel_gg,
   0,(UBYTE *)"Etiketten-Druck-Programm - ETI 1.3",
...
{
   static struct StringInfo clr_si = { 0 };
...
struct Gadget *ggp;
{
   ...
   if (ggp->Flags & GRELBOTTOM) {
      ...
      RectFill(wnd->RPort,(long)(ggp->LeftEdge + 50 * 8),
            (long)(ggp->TopEdge),
...
char **argv;
{
   ...
   /* parameter laden */
   if (fp = fopen("eti.parms","r")) {
      fscanf(fp,"%d%d%d%ld%ld%ld",&papier_laengen[0],&papier_laengen[1],
            &papier_laengen[2],&delays[0],&delays[1],&delays[2]);
      fclose(fp);
   } else
      puts("Unable to open the parameter file!");
   ...
   do {
      ...
      if (imsg.Class == GADGETUP) {
         ...
         if (l && ggp->GadgetID < 18) ActivateGadget(ggp->NextGadget,wnd,0L);
         switch (ggp->GadgetID) {
            case 19:
               install_stop();
               ...
               print();
               if (!test_stop()) {
                  message("Der Testausdruck wird erstellt...");
                  prt();
               }
               remove_stop();
               break;
            ...
            case 21:
               if (anzahl_info.LongInt &&
                  ...
                  print();
                  for (i = 1; i <= l; ++i) {
                     if (i != 1 && delays[groesse - 1]) {
                        Delay(delays[groesse - 1]);
                     }
                     if (test_stop()) break;
            ...
            case 23:
               ...
               if (groesse == 1) {
                  for (i = 0; i < 4; ++i) {
                     ch_sv[i] = gefahren[i][50];
                     ch_sv[i + 4] = hinweise[i][50];
                  }
                  sv_flg = 1;
                  l = 51;
                  ...
                  if (sv_flg) {
                     for (i = 0; i < 4; ++i) {
                        gefahren[i][50] = ch_sv[i];
                        hinweise[i][50] = ch_sv[i + 4];
                     }
                  }
                  l = 91;
               }
               if (groesse != 3) {
                  for (i = 0; i < 8; ++i) {
                     clear_si(info_tab[i]);
                     info_tab[i]->Buffer[(info_tab[i]->MaxChars =
                           l) - 1] = 0;
                  }
               }
